...
class PatternEncoder(nn.Module):
    def __init__(self,pattern,use_pattern_ib=False,pattern_ib_prior=0.05,pattern_ib_temperature=1.0,
                 pattern_ib_gate="gumbel_sigmoid",pattern_ib_topk=0,pattern_ib_eps=1e-8,
                 meta_ib_dim=0,meta_ib_modulate_pattern_query=False):
        ...
        self.use_pattern_ib = use_pattern_ib
        self.pattern_ib_prior = pattern_ib_prior
        self.pattern_ib_temperature = pattern_ib_temperature
        self.pattern_ib_gate = pattern_ib_gate
        self.pattern_ib_topk = pattern_ib_topk
        self.pattern_ib_eps = pattern_ib_eps
        self.meta_ib_modulate_pattern_query = meta_ib_modulate_pattern_query and meta_ib_dim > 0
        if self.meta_ib_modulate_pattern_query:
            self.meta_film = nn.Linear(meta_ib_dim, 2 * self.D)
            nn.init.zeros_(self.meta_film.weight)
            nn.init.zeros_(self.meta_film.bias)

    def _zero_ib_loss(self, X):
        return torch.zeros((), device=X.device, dtype=X.dtype)

    def _modulate_query(self, qry, u_tau):
        if u_tau is None or not self.meta_ib_modulate_pattern_query:
            return qry
        gamma, beta = self.meta_film(u_tau).chunk(2, dim=-1)
        while gamma.dim() < qry.dim():
            gamma = gamma.unsqueeze(1)
            beta = beta.unsqueeze(1)
        return (1.0 + gamma) * qry + beta

    def _pattern_ib(self, score, training_ib):
        eps = max(float(self.pattern_ib_eps), float(torch.finfo(score.dtype).eps))
        pi = torch.sigmoid(score / self.pattern_ib_temperature).clamp(eps, 1.0 - eps)
        p0 = min(max(float(self.pattern_ib_prior), eps), 1.0 - eps)
        one_minus_pi = (1.0 - pi).clamp_min(eps)
        pattern_ib_loss = pi * torch.log(pi / p0) + one_minus_pi * torch.log(one_minus_pi / (1.0 - p0))
        pattern_ib_loss = pattern_ib_loss.mean()

        if self.pattern_ib_gate == "none":
            return None, pattern_ib_loss
        if self.pattern_ib_gate == "soft":
            return pi, pattern_ib_loss
        if self.pattern_ib_gate == "gumbel_sigmoid" and training_ib:
            u1 = torch.rand_like(pi).clamp(eps, 1.0 - eps)
            u2 = torch.rand_like(pi).clamp(eps, 1.0 - eps)
            logistic_noise = torch.log(u1) - torch.log(u2)
            logit_pi = torch.log(pi) - torch.log(1.0 - pi)
            gate = torch.sigmoid((logit_pi + logistic_noise) / self.pattern_ib_temperature)
            return gate, pattern_ib_loss

        gate = pi
        if self.pattern_ib_topk > 0:
            topk = min(int(self.pattern_ib_topk), gate.shape[-1])
            _, idx = torch.topk(gate, k=topk, dim=-1)
            mask = torch.zeros_like(gate).scatter_(-1, idx, 1.0)
            gate = gate * mask
        return gate, pattern_ib_loss

    def _apply_pattern_gate(self, score, weight, training_ib):
        gate, pattern_ib_loss = self._pattern_ib(score, training_ib)
        if gate is None:
            return weight, pattern_ib_loss
        gated_weight = weight * gate
        weight = gated_weight / (gated_weight.sum(dim=-1, keepdim=True) + self.pattern_ib_eps)
        return weight, pattern_ib_loss

    def forward(self, X, return_ib_loss=False, training_ib=None, meta_context=None, u_tau=None, return_aux=False):
        """
        ...
        # qry : [B, N, L, D]
        qry = self.Qnet(X)
        if meta_context is not None and u_tau is None:
            u_tau = meta_context
        qry = self._modulate_query(qry, u_tau)
        
        score = torch.einsum('kd,bnld->bnlk',pattern,qry) / self.temporature
        # print(weight.shape)
        # [B, N, L, D]
        weight = F.softmax(score,dim = 3)
        pattern_ib_loss = self._zero_ib_loss(X)
        if self.use_pattern_ib:
            if training_ib is None:
                training_ib = self.training
            weight, pattern_ib_loss = self._apply_pattern_gate(score, weight, training_ib)
        # print(weight.shape)
        # [B, N, L, D]
        w_X = torch.einsum('bnlk,kd->bnld',weight,self.pattern)
        # print(w_X.shape)
        if return_aux and return_ib_loss:
            return w_X, pattern_ib_loss, {"pattern_weight": weight}
        if return_aux:
            return w_X, {"pattern_weight": weight}
        if return_ib_loss:
            return w_X, pattern_ib_loss
        return w_X
...
class PatternEncoder_patternkeyv2(PatternEncoder):
    def __init__(self,pattern,use_pattern_ib=False,pattern_ib_prior=0.05,pattern_ib_temperature=1.0,
                 pattern_ib_gate="gumbel_sigmoid",pattern_ib_topk=0,pattern_ib_eps=1e-8,
                 meta_ib_dim=0,meta_ib_modulate_pattern_query=False):
    ...
    def forward(self, X, return_ib_loss=False, training_ib=None, meta_context=None, u_tau=None, return_aux=False):
        ...
        # weight = torch.einsum('kd,bnld->bnlk',pattern,qry) / self.temporature
        score = torch.einsum('dk,bnld->bnlk',self.qry_mat,qry)
        # print(weight.shape)
        # [B, N, L, D]
        weight = F.softmax(score,dim = 3)
        pattern_ib_loss = self._zero_ib_loss(X)
